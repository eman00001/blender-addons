bl_info = {
    "name": "Lyrics Animated Creator (Alpha Fade, Custom Font)",
    "author": "news0cks",
    "version": (3, 3),
    "blender": (4, 5, 0),
    "location": "View3D > Sidebar > Lyrics Creator",
    "description": "Create text objects for lyrics with alpha fade animation, NLA strips, and custom font",
    "category": "Animation",
}
...
class NEWS0CKS_OT_CreateLyrics(bpy.types.Operator):
    bl_idname = "news0cks.create_lyrics"
    bl_label = "Create Lyrics"
    bl_description = "Create animated text objects for lyrics with alpha fade and NLA strips"
    ...
    def execute(self, context):
        ...
        for offset, line in enumerate(lyrics):
            ...
            # Animate object property instead of material
            obj["lyric_alpha"] = 0.0
            ...
            action = obj.animation_data.action or bpy.data.actions.new(name=f"{obj.name}_action")
            obj.animation_data.action = action
            track = obj.animation_data.nla_tracks.new()
            strip = track.strips.new(name=f"Strip_{obj.name}", start=frame_start, action=action)
            strip.frame_end = frame_visible_end + fade
            obj.animation_data.action = None  # NLA drives animation

        self.report({'INFO'}, f"Created {len(lyrics)} text objects in '{coll_name}' with alpha fade NLA strips")
        return {'FINISHED'}

class NEWS0CKS_PT_LyricPanel(bpy.types.Panel):
    bl_label = "Lyrics Animated Creator"
    bl_idname = "NEWS0CKS_PT_LyricPanel"
    ...
    def draw(self, context):
        ...
        layout.operator("news0cks.create_lyrics")

classes = [NEWS0CKS_Props, NEWS0CKS_OT_CreateLyrics, NEWS0CKS_PT_LyricPanel]
